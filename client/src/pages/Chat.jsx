...
const Chat = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await axios.post(
        `${import.meta.env.VITE_SERVER_URL}/api/chat/getCareerGuidance`,
  ...
  const markdownComponents = {
    ...
    h4: ({ children }) => (
      <h4 className="text-base font-semibold text-blue-300 mb-2 mt-3 first:mt-0">
        {children}
      </h4>
    ),
    h5: ({ children }) => (
      <h5 className="text-sm font-semibold text-blue-300 mb-2 mt-3 first:mt-0">
        {children}
      </h5>
    ),
    h6: ({ children }) => (
      <h6 className="text-xs font-semibold text-blue-300 mb-2 mt-3 first:mt-0">
        {children}
      </h6>
    ),
    ...
    li: ({ children, ...props }) => {
      ...
      return (
        <li className={`flex items-start gap-3 text-gray-100 leading-relaxed`}>
          {!isOrdered && (
    ...
    th: ({ children }) => (
      <th className="border border-gray-600 px-4 py-2 text-left font-semibold text-blue-200">
        {children}
      </th>
    ),
    td: ({ children }) => (
